...
| **Railway** | Hosts the backend that processes everything above (parses PDFs, runs **local embeddings**, proxies Anthropic) | Compute | Operational logs may transiently contain a user_id and query string. |
| **Vercel** | Serves the frontend; standard request logs | Web hosting | No application data stored. |

## Embeddings are LOCAL

PDF and paper text is embedded **on our own Railway backend** using a local model
(`sentence-transformers/all-MiniLM-L6-v2`). **Text is never sent to a third-party
embedding service.** The resulting vectors are stored in Supabase (pgvector).
